...
import { Settings, Gamepad2, Play, Film, User, FileText, Globe, Briefcase, Image as ImageIcon } from 'lucide-react';
import curriculoPdf from '../../assets/curriculo.pdf';
import { Projects } from '../apps/Projects';

// Importação dos Assets (MANTIDA)
...
const DesktopIcon = ({ id, label, icon, onDoubleClick, onContextMenu, isRenaming, onRename }) => {
    ...
    return (
        <div onDoubleClick={onDoubleClick} onContextMenu={onContextMenu} className="flex flex-col items-center justify-start gap-1 p-1 w-[86px] h-[100px] hover:bg-[#316ac5]/50 border border-transparent hover:border-[#316ac5]/30 rounded-[3px] cursor-pointer group transition-none mb-2">
            <div className="w-10 h-10 md:w-[48px] md:h-[48px] flex-shrink-0 flex items-center justify-center filter drop-shadow-[2px_2px_1px_rgba(0,0,0,0.3)]">{icon}</div>
...
export const Desktop = () => {
    ...
    // ESTADO PARA O PISCAR (Visível ou Invisível)
    const [isVisible, setIsVisible] = useState(true);

    // EFEITO DE ATUALIZAR (Simula o flicker do Windows)
    useEffect(() => {
        if (refreshKey > 0) {
            setIsVisible(false); // Some
            const timer = setTimeout(() => setIsVisible(true), 50); // Volta rápido (50ms)
    ...
    return (
        <div className="absolute inset-0 p-2 flex flex-col flex-wrap content-start items-start gap-y-1 gap-x-2 z-0 pt-4 pl-2 pb-12 overflow-hidden" onContextMenu={(e) => openContextMenu(e, 'desktop')}>
            ...
            {/* Se isVisible for false, esconde tudo (flicker) */}
            {isVisible && (
                <div className="contents">
                    ...
                    <DesktopIcon id="video_demo" label={getName('video_demo', "Vídeo Demo")} icon={<XP_VideoNewIcon />} isRenaming={renamingId === 'video_demo'} onRename={renameItem} onDoubleClick={() => openWindow('video-demo', 'Apresentação', <Film size={16} className="text-purple-500" />, <VideoPlayer src={videoUrl} title="Apresentação do Projeto" />, '/', { isSkin: true })} onContextMenu={(e) => openContextMenu(e, 'file', 'video_demo')} />
